...
export default function Signin() {
  ...
  return (
    <div className='relative min-h-dvh w-full flex items-center justify-center overflow-x-hidden overflow-y-auto font-mono text-cyan-50 bg-[#050505] selection:bg-fuchsia-500/30 py-6 sm:py-0'>
      ...
      <TransitionLink 
        href="/" 
        className="fixed top-6 left-6 z-50 flex items-center gap-2 px-6 py-2.5 bg-black/30 backdrop-blur-md border border-white/10 rounded-full hover:border-cyan-500/50 hover:bg-cyan-950/40 transition-all duration-300 shadow-lg group cursor-pointer overflow-hidden"
      >
                    ...
                    <form onSubmit={signinHandler} className='flex flex-col gap-3'>
                        ...
                        <div className="flex items-center justify-between mt-1 px-1">
                            <label className="flex items-center gap-2 cursor-pointer group/chk select-none">
                                <div className="relative w-3.5 h-3.5 border border-cyan-500 md:border-cyan-800 bg-[#080c14] rounded-[2px]">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <button type="button" className="flex items-center justify-center gap-2 h-9 bg-[#0a0e17] border border-cyan-900/30 hover:border-cyan-500/60 transition-all duration-300 group [clip-path:polygon(0_0,100%_0,100%_100%,10px_100%,0_calc(100%-10px))] cursor-pointer" onClick={() => signIn('google', { callbackUrl: '/' })}><FaGoogle className='text-sm' /><span className='font-orbitron text-xs font-semibold tracking-wider'>GOOGLE</span></button>
                          <button type="button" className="flex items-center justify-center gap-2 h-9 bg-[#0a0e17] border border-cyan-900/30 hover:border-cyan-500/60 transition-all duration-300 group [clip-path:polygon(0_0,100%_0,100%_calc(100%-10px),calc(100%-10px)_100%,0_100%)] cursor-pointer" onClick={() => signIn('github', { callbackUrl: '/' })}><FaGithub className='text-sm' /><span className='font-orbitron text-xs font-semibold tracking-wider'>GITHUB</span></button>
                        </div>
                        <div className="text-center mt-2">
                            <p className="font-rajdhani text-sm sm:text-xs text-gray-500 font-bold uppercase tracking-wide">
                                No Access ID? 
                                <TransitionLink href="/register" className='font-orbitron text-purple-400 hover:text-purple-300 ml-2 hover:underline underline-offset-4 decoration-purple-500/50 transition-colors'>Register User</TransitionLink>
                            </p>
                        </div>
                    </form>
                </div>
